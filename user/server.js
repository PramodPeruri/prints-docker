...
// Graceful shutdown
process.on("SIGINT", async ()=>{try{if(global.mongoClient) await global.mongoClient.close();}catch(e){} try{if(global.redisClient) await global.redisClient.quit();}catch(e){} process.exit(0);});
process.on("SIGTERM", async ()=>{try{if(global.mongoClient) await global.mongoClient.close();}catch(e){} try{if(global.redisClient) await global.redisClient.quit();}catch(e){} process.exit(0);});
